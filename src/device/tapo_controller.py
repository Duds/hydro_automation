...
class TapoController:
    ...
    def __init__(self, ip_address: str, email: str, password: str, logger=None, enable_auto_discovery: bool = True):
        ...
        self._last_successful_operation = None  # Track last successful operation time
        self._consecutive_failures = 0  # Track consecutive failures for health monitoring
        self._operation_timeout = 45  # Increased timeout for operations (was 30)
    ...
    def _run_async(self, coro, timeout: Optional[float] = None):
        ...
        Args:
            ...
            timeout: Timeout in seconds (defaults to self._operation_timeout)
        ...
        timeout_value = timeout if timeout is not None else self._operation_timeout
    ...
    def discover_device(self, timeout: int = 10) -> Optional[str]:
        ...
        async def _discover_async():
            ...
            try:
                ...
                devices_to_close = []  # Track devices that need cleanup
                ...
                try:
                    for discovered_device in discovered_devices:
                        ...
                        try:
                            ...
                            if isinstance(device, TapoPlug):
                                ...
                                # Close this device's session
                                await device.client.close()
                                # Close any other devices we tried
                                for d in devices_to_close:
                                    try:
                            ...
                            # Ensure we close the device session even on error
                            if device:
                                try:
                                    await device.client.close()
                                except Exception:
                    ...
                    # Ensure all devices are closed on error
                    for device in devices_to_close:
                        try:
                            await device.client.close()
                        except Exception:
        ...
        try:
            # Use thread-safe async execution with extended timeout for discovery
            return self._run_async(_discover_async(), timeout=timeout + 30)
        except Exception as e:
            if self.logger:
    ...
    def connect(self, max_retries: int = 3, retry_delay: float = 2.0) -> bool:
        """
        Establish connection to the Tapo device with exponential backoff.
        ...
        async def _connect_async(ip_addr: str):
            ...
            for attempt in range(1, max_retries + 1):
                ...
                try:
                    ...
                    # Ensure device session is closed on error
                    if device:
                        try:
                            await device.client.close()
                        except Exception:
                    ...
                    if attempt < max_retries:
                        ...
                        if self.logger:
                            self.logger.debug(f"Waiting {backoff_delay:.1f}s before retry (exponential backoff)")
        ...
        try:
            ...
            if self._run_async(_connect_async(self.ip_address), timeout=60):
                return True
            ...
            if self.enable_auto_discovery:
                ...
                if discovered_ip:
                    ...
                    # Try connecting to discovered IP with exponential backoff
                    if self._run_async(_connect_async(discovered_ip), timeout=60):
                        return True
                else:
                    if self.logger:
    ...
    def _check_connection_health(self) -> bool:
        ...
        if self._consecutive_failures >= 3:
            if self.logger:
                self.logger.warning(
                    f"Connection health check: {self._consecutive_failures} consecutive failures. "
                    "Attempting to reconnect..."
                )
            # Close existing connection
            self.close()
            # Attempt to reconnect
            return self.connect(max_retries=3, retry_delay=2.0)
        
        return True
    
    def _reconnect_if_needed(self) -> bool:
        """
        Reconnect to device if connection is lost.
    ...
    def turn_on(self, verify: bool = True, max_retries: int = 3) -> bool:
        """
        Turn on the device with automatic reconnection on failure.
        ...
        if not self._reconnect_if_needed():
            if self.logger:
                self.logger.error("Cannot turn on device: not connected and reconnection failed")
            return False
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    if result.is_success():
                        ...
                        if verify:
                            ...
                            try:
                                ...
                                if self.logger:
                                    self.logger.warning(f"Failed to verify device state: {e}")
                                # If verification fails but command succeeded, consider it a partial success
                                if attempt == max_retries:
                                    self._last_successful_operation = time.time()
                    ...
                    if "connection" in str(e).lower() or "timeout" in str(e).lower():
                        if attempt < max_retries:
                            if self.logger:
                                self.logger.debug("Connection error detected, attempting reconnection...")
                            # Try to reconnect
                            self.connected = False
                            if not self._reconnect_if_needed():
                                if attempt == max_retries:
        ...
        try:
            ...
            if self.logger:
                ...
                if isinstance(e, (TimeoutError, concurrent.futures.TimeoutError)):
                    self.logger.error(
                        f"Error turning device on: Timeout after {self._operation_timeout}s. "
    ...
    def turn_off(self, verify: bool = True, max_retries: int = 3) -> bool:
        """
        Turn off the device with automatic reconnection on failure.
        ...
        if not self._reconnect_if_needed():
            if self.logger:
                self.logger.error("Cannot turn off device: not connected and reconnection failed")
            return False
        ...
        try:
            ...
            if self.logger:
                ...
                if isinstance(e, (TimeoutError, concurrent.futures.TimeoutError)):
                    self.logger.error(
                        f"Error turning device off: Timeout after {self._operation_timeout}s. "
                        f"Device may be unresponsive."
                    )
                else:
    ...
    def is_device_on(self) -> Optional[bool]:
        """
        Check if device is currently on with timeout recovery.
        ...
        async def _check_state_async():
            try:
                ...
                if self.logger:
                    self.logger.warning(f"Failed to get device state: {type(e).__name__}: {e}")
                # If it's a connection error, mark connection as lost
                if "connection" in str(e).lower() or "timeout" in str(e).lower():
                    self.connected = False
                return None

        try:
            # Use thread-safe async execution with shorter timeout for state checks
            return self._run_async(_check_state_async(), timeout=20)
        except Exception as e:
            if self.logger:
                import concurrent.futures
                ...
                if isinstance(e, (TimeoutError, concurrent.futures.TimeoutError)):
                    self.logger.warning(
                        f"Error checking device state: Timeout after 20s "
                        f"(device may be unresponsive or network issue). "
                    ...
                    self.logger.warning(
                        f"Error checking device state: {error_type}: {error_msg}"
                    )
                    self._consecutive_failures += 1
            return None
        ...
        if self.device and self.connected:
            async def _close_async():
                try:
                    ...
                    if hasattr(self.device, 'client') and self.device.client:
            ...
            try:
                # Use thread-safe async execution with timeout
                self._run_async(_close_async(), timeout=10)
            except Exception as e:
                if self.logger:
